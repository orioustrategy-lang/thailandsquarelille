...
export default function Restaurants() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col lg:flex-row pt-20 sm:pt-[120px]">
        ...
        <div className="w-full lg:w-[420px] bg-white flex flex-col border-r border-gray-200">
          ...
          <div className="p-3 sm:p-4 border-b border-gray-100">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 sm:pl-10 pr-4 py-2.5 sm:py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm sm:text-base"
              />
            </div>
            <button className="mt-2 sm:mt-3 text-amber-600 hover:text-amber-700 text-xs sm:text-sm font-semibold flex items-center gap-2 hover:underline">
          ...
          <div className="flex-1 overflow-y-auto max-h-[40vh] lg:max-h-none">
            {restaurants.map((restaurant) => (
              <div
                ...
                className={`p-3 sm:p-4 border-b border-gray-100 cursor-pointer transition-all hover:bg-amber-50 ${
                  selectedId === restaurant.id ? "bg-amber-50 border-l-4 border-l-amber-500" : ""
                }`}
              >
                <div className="flex justify-between items-start mb-1.5 sm:mb-2">
                  <h3 className="font-bold text-slate-900 text-xs sm:text-sm uppercase">
                    {restaurant.name}
                  </h3>
                  <div className="text-right">
                    <span className="text-[10px] sm:text-xs text-slate-500 block">{restaurant.status}</span>
                    <span className="text-xs sm:text-sm font-semibold text-amber-600">{restaurant.openTime}</span>
                  </div>
                </div>
                <p className="text-xs sm:text-sm text-slate-600">{restaurant.address}</p>
                <p className="text-xs sm:text-sm text-slate-600">{restaurant.city}</p>
                <p className="text-[10px] sm:text-xs text-slate-400 mt-1">{restaurant.distance}</p>
              </div>
            ))}
          </div>
          ...
          <div className="p-3 sm:p-4 border-t border-gray-100">
            <button className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-semibold px-3 sm:px-4 py-2 rounded-full transition-colors text-sm">
        ...
        {/* Right Side - Map */}
        <div className="flex-1 relative bg-slate-100 min-h-[350px] sm:min-h-[500px] lg:min-h-0">
          {/* Map Type Toggle */}
          <div className="absolute top-3 sm:top-4 left-3 sm:left-4 z-[1000] bg-white rounded-lg shadow-md overflow-hidden flex">
            <button
